...
namespace WzorceProjektowe
{
  ...
  class NewYorkTimes : IObserver 
  {
        public void Update(ISubject subject) 
    {
      if ((subject as NewsAgency).State == Genre.Economy || (subject as NewsAgency).State == Genre.Science || (subject as NewsAgency).State == Genre.Politics || (subject as NewsAgency).State == Genre.Sport)
            {
